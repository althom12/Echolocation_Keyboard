...
public class ObstacleManager : MonoBehaviour, IObstacleService
{
    ...
    /// This manager holds references to scene objects (obstacleSets) that would
    ...
    /// <summary>
    /// Tracks the currently active preset index (0-5), or -1 if none.
    /// </summary>
    private int activePresetIndex = -1;

    /// <summary>
    /// Tracks the currently active custom column index (6-8), or -1 if none.
    /// </summary>
    ...
    private void Awake()
    {
        ...
        {
            Debug.LogWarning($"[ObstacleManager] Duplicate instance found on '{gameObject.name}'. Destroying duplicate.");
            Destroy(gameObject);
            return;
        }
    }
    ...
    /// <summary>
    /// Returns the root GameObject of the currently active obstacle set.
    /// Optimization: Used by ObstacleLocatorAudio to avoid searching the whole scene.
    /// </summary>
    public GameObject GetActiveSetObject()
    {
        int index = GetActiveIndex();
        // Check if index is valid (not -1) and within array bounds
        if (index != -1 && index < obstacleSets.Length)
        {
    ...
    /// <summary>
    /// Handles selection of "None" (index -1).
    ...
    /// <summary>
    /// Handles selection of baked presets (indices 0-5).
    ...
    /// <summary>
    /// Handles selection of custom columns (indices 6-8).
    ...
    private void ApplyMaterial(int materialIndex)
    {
        ...
        // Call the material controller's ApplyMaterial method
    ...
    // ???????????????????????????????????????????????????????????
    // PUBLIC API - Query Methods (IObstacleService Implementation)
    // ???????????????????????????????????????????????????????????
